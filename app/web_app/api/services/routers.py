...
def get_services():
    query: ServiceListQuerySchema = request.context.query
    service_repo: ServiceRepository = g.service_repo

    services = service_repo.get_services(user_id=int(get_jwt_identity()), is_active=query.is_active)
...
def create_service():
    ...
    service = operation(dto=CreateServiceDTO(
        name=body.name,
        url=str(body.url),
        type=body.type,
...
def update_service(service_id):
    body: ServiceUpdateSchema = request.context.body
    operation: UpdateService = g.update_service

    service = operation(dto=UpdateServiceDTO(
...
def delete_service(service_id):
    ...
    operation(dto=DeleteServiceDTO(
        service_id=service_id,
        user_id=int(get_jwt_identity())
    ))
